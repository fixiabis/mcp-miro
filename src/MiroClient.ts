...
export class MiroClient {
	...
	async deleteStickyNote(boardId: string, itemId: string) {
		return this.fetchApi(`/boards/${boardId}/sticky_notes/${itemId}`, {
			method: "DELETE",
		}) as Promise<void>;
	}
	...
	async bulkCreateItems(
		...
		return this.fetchApi(`/boards/${boardId}/items/bulk`, {
			method: "POST",
			body: items,
		}) as Promise<any>;
	}
	...
	async getItemsOnBoard(boardId: string, limit: number = 10, cursor?: string, type?: string): Promise<MiroItem[]> {
		return this.fetchApi(
			`/boards/${boardId}/items?limit=${limit}${cursor ? `&cursor=${cursor}` : ""}${type ? `&type=${type}` : ""}`
		) as Promise<MiroItem[]>;
	}
}
